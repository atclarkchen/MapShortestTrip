...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        Road firstRoad = _map.getLabel(from, segment.get(1));

        double distSoFar = firstRoad.length();
        Location finalDest = _map.getLabel(segment.get(segment.size() - 1));
        String destName = finalDest.toString();

        for (int i = 2; i < segment.size(); i += 1) {
            Road currRoad = _map.getLabel(segment.get(i - 1), segment.get(i));
            if (firstRoad.toString().equals(currRoad.toString())
                && firstRoad.direction().equals(currRoad.direction())) {
                distSoFar += currRoad.length();

                if (i == segment.size() - 1) {
                    System.out.printf("%d. Take %s %s for %.1f miles to %s.\n",
                        seq, firstRoad.toString(),
                        firstRoad.direction().fullName(), distSoFar, destName);
                    seq += 1;
                    return seq;
                }
            } else {
                if (i == segment.size() - 1) {
                    System.out.printf("%d. Take %s %s for %.1f miles.",
                        seq, firstRoad.toString(),
                        firstRoad.direction().fullName(), distSoFar);
                    seq += 1;

                    System.out.println();
                    System.out.printf("%d. Take %s %s for %.1f miles to %s.\n",
                        seq, currRoad.toString(),
                        currRoad.direction().fullName(),
                        currRoad.length(), destName);
                    seq += 1;
                    return seq;
                } else {
                    System.out.printf("%d. Take %s %s for %.1f miles.\n",
